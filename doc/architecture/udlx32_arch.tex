...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{pictures/architecture_interface.eps}
  \end{figure}
...
This instructions realize registers operations, most operations are arithmetic. All arithmetic opcodes are zero, differing only in the function value.
  ...
  \begin{table}[H]
    \begin{center}
	 ...
	 100100/0x24 	& and & $R_D = R_S1 and R_S2$ \\ \hline
	 100101/0x25 & or & $R_D = R_S1 or R_S2$ \\ \hline
	 011000/0x18 & mult & $R_D = R_S1 x R_S2$ \\ \hline
	 011010/0x1A & div & $R_D = R_S1 / R_S2$ \\ \hline
	 011100/0x1C & cmp & $R_D = R_S1 cmp R_S2$ \\ \hline
	 011101/0x1D & not & $R_D = R_S1 not R_S2$ \\ \hline
...
Instructions related with instruction memory jump. The instructions are JPC (J),RET (RFE), CALL (TRAP).
  ...
  \begin{table}[H]
    \begin{center}
	 ...
	 111110/0x3e 	& call(trap) & $trap = 1;   EPC = PC;   PC = SISR; ESR = SR;   ECA = masked CA;   SR = 0; EDATA = Sext(imm);clear CA but catch new interrupt events$ \\ \hline
	 111111/0x3f 	& ret(rfe) & $SR = ESR;   PC = EPC$ \\ \hline
...
instruction time.
    ...
    \begin{center}
      \begin{longtable}[pos]{| l | c | c | m{7cm} |} \hline       	
        ...
        \endlastfoot
      	...
      	w\_reg\_addr\_out  & TBD   & output   & GPR bank write enable signal \\ \hline
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{pictures/datapath/id_datapath.eps}
  \end{figure} 
...
\subsubsection{Internal Datapath} 
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{pictures/datapath/ex_datapath.eps}
  \end{figure}      
...
The memory map shown was created with separate address to show that devices have no relation between them and to show that the memory used in this project will be a small fraction for the 4GBytes possible.
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{pictures/blocks/sram_block.eps}
  \end{figure}  
  ...
  \FloatBarrier
    \begin{center}
      \begin{longtable}[pos]{| l | c | c | m{7cm} |} \hline         
        ...
        sram\_rd\_en          & 1 & output  & ??? \\ \hline
        sram\_rd\_data        & 16   & output & ???  \\ \hline
        sram\_addr           & 10  & output & Memory address that goes to the SRAM  \\ \hline
        sram\_wr\_en    & 1  & output  & ???  \\ \hline
        sram\_wr\_data    & 16  & output  & ???  \\ \hline
      \end{longtable}
    \end{center} 
  ...
  \FloatBarrier
    \begin{center}
      \begin{longtable}[pos]{| l | c | c | m{7cm} |} \hline         
        ...
        sdram\_ldqm    & 1  & output  & ???  \\ \hline
        sdram\_udqm    & 1  & output  & ???  \\ \hline
  ...
  \FloatBarrier
    \begin{center}
      \begin{longtable}[pos]{| l | c | c | m{7cm} |} \hline         
        ...
        instruction\_decoded               & 1   & input  & ???    \\ \hline
        alu\_src\_out\_n               & 1   & output  & ???    \\ \hline
        alu\_opcode\_out           & 1  & output  & ??? \\ \hline
        mem\_data\_wr\_en\_out           & 1  & output  & ??? \\ \hline
        write\_back\_mux\_sel\_out      & 1   & output  & ??? \\ \hline
        w\_reg\_wr\_en\_out        & 1   & output  & ???  \\ \hline
        w\_reg\_addr\_out  & 1 & output  & ??? \\ \hline
        branch\_inst\_out          & 12 & output  & ??? \\ \hline
...
The ROM must have the DLX reset PC register address. When the system is reseted the processor will read instructions from the ROM address. The code inside the ROM will copy the instruction and data code from a static memory to the instruction memory SRAM and the data memory SDRAM.
  \begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{pictures/bootloader.eps}
  \end{figure}  
